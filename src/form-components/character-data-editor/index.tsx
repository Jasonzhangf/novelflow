...
const setNestedValue = (obj: Record<string, any>, path: string[], newValue: any): Record<string, any> => {
...
const MemoizedRenderPropertyValue = memo(function RenderPropertyValue({
  ...
  const displayKey = itemValue && typeof itemValue === 'object' && 'Caption' in itemValue 
    ? itemValue.Caption 
    : itemKey;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '5px', paddingLeft: isBasicInfo ? '0px' : '10px' }}>
      <Text strong style={{ minWidth: '150px', maxWidth: '200px', marginRight: '10px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      {isEditing ? (
        <>
          <Input
            ...
            autoFocus
          />
          <Button onClick={handleSave} size="small" style={{ marginRight: '5px' }}>保存 / Save</Button>
          <Button onClick={() => {
            setLocalEditValue(getDisplayValue());
            setIsEditing(false);
          }} size="small" type="tertiary">取消 / Cancel</Button>
        </>
      ) : (
        <Text 
        ...
        >
          {displayValue || <span style={{ fontStyle: 'italic', color: '#999' }}>未设置 / Not set</span>}
        </Text>
      )}
    </div>
  );
...
const MemoizedRenderObjectRecursive = memo(function RenderObjectRecursive({
  ...
  const header = (
    ...
    >
      {isOpen ? <IconChevronDown style={{ marginRight: '8px'}} /> : <IconChevronRight style={{ marginRight: '8px'}} />}
      <Title heading={5} style={{ margin: 0 }}>{groupKey}</Title>
    </div>
  );
      ...
      {isOpen && (
        <div style={{ paddingLeft: '20px' }}>
          ...
          {childEntries.map(([key, val]) => {
            ...
            if (val && typeof val === 'object' && !('Value' in val && 'Caption' in val) && !(val instanceof Array)) {
              return (
            ...
            // Handle arrays
              ...
              return (
                <div key={`${groupKey}-${key}-array`} style={{ marginBottom: '5px', paddingLeft: '10px' }}>
                  <Text strong>{key}: </Text>
                  {!val || val.length === 0 ? (
                    <Text type="secondary" style={{ marginLeft: '10px' }}>(Empty / 空)</Text>
                  ) : (
                    <pre style={{ fontSize: '0.9em', backgroundColor: '#f8f8f8', padding: '10px', borderRadius: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
  ...
  if (Object.keys(dataToRender).length === 0) {
    return (
      <div style={{ padding: '10px', textAlign: 'center' }}>
        ...
        <br />
        <Text type="secondary">Please import a character file or add data manually. / 请导入角色文件或手动添加数据。</Text>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '10px' }}>
      {keysToRender.map((topKey) => {
        ...
        if (!topVal && !isBasicInfoKey) {
          return (
            <div key={`empty-${topKey}`} style={{ marginBottom: '15px' }}>
              <Title heading={5} style={{ margin: '8px 0' }}>{topKey}</Title>
        ...
        if (typeof topVal === 'object' && topVal !== null && !(topVal instanceof Array) && !('Value' in topVal && 'Caption' in topVal)) {
          return (
            <MemoizedRenderObjectRecursive
              key={`group-${topKey}`}
              groupKey={topKey}
          ...
          return (
            <div key={`array-${topKey}`} style={{ marginTop: '10px', marginBottom: '15px' }}>
              ...
              {(!topVal || topVal.length === 0) ? (
                <Text type="secondary" style={{ marginLeft: '20px' }}>(Empty / 空)</Text>
              ) : (
                <pre style={{ fontSize: '0.9em', backgroundColor: '#f8f8f8', padding: '10px', borderRadius: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', marginLeft: '20px' }}>
                  {JSON.stringify(topVal, null, 2)}
                </pre>
              )}
              {topKey === 'relationships' && (
                <Button size="small" style={{marginTop: '5px', marginLeft: '20px'}} onClick={() => console.log('TODO: Add to', topKey)}>
