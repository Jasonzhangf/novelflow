...
const PropertiesPanelCharacterFields: React.FC = () => {
  ...
  const handleLoadFile = async () => {
    if (!selectedNode || !formModel || !localCharacterFilePath) {
      console.warn('Cannot load file: No node selected, formModel unavailable, or file path is empty.');
      return;
    }
    ...
    try {
      ...
      if (localCharacterFilePath === 'Doc/李观一.json') {
        fileContentString = JSON.stringify({
          name: "Li Guanyi / 李观一",
          age: 13,
          background: { origin: "Unknown", occupation: "Apothecary Apprentice", history: "Survived assassination attempt..." },
          personality: { CoreTemperament: { OptimismLevel: { Value: 60, Caption: "乐观度" } } },
          relationships: [{ character: "Murong Qiushui", type: "Aunt (Guardian)" }],
          language: "chinese",
        });
      } else {
        throw new Error(`Simulated file read: File not found or access denied for ${localCharacterFilePath}. Please implement actual file reading.`);
      }
